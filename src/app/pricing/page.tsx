...
const plans: Plan[] = [
  {
    id: 'basic',
    name: 'Basic Plan',
    description: 'Perfect for small businesses getting started with conversion optimization',
    features: [
  ...
  },
  {
    id: 'pro',
    name: 'Pro Plan',
    description: 'For growing businesses that need advanced conversion optimization',
    features: [
    ...
    <div className="bg-white dark:bg-zinc-950">
      <div className="mx-auto max-w-7xl px-6 py-24 sm:py-32 lg:px-8">
        ...
        <div className="mx-auto max-w-4xl text-center">
          <Heading level={1} className="text-4xl font-bold tracking-tight sm:text-5xl">
            Choose Your Plan
          </Heading>
        ...
        <div className="mx-auto mt-16 grid max-w-lg grid-cols-1 items-center gap-y-6 sm:mt-20 sm:gap-y-0 lg:max-w-4xl lg:grid-cols-2">
          {plans.map((plan, planIdx) => (
            ...
            >
              ...
              <Subheading level={3} className={plan.popular ? 'text-white dark:text-gray-900' : ''}>
                {plan.name}
              </Subheading>
              
              <Text className={`mt-4 ${plan.popular ? 'text-gray-300 dark:text-gray-600' : ''}`}>
                {plan.description}
              </Text>
              
              <p className="mt-6 flex items-baseline gap-x-1">
                <span className={`text-4xl font-bold tracking-tight ${
                  plan.popular ? 'text-white dark:text-gray-900' : 'text-gray-900 dark:text-white'
                }`}>
                  {formatPrice(getPrice(plan))}
                </span>
                <span className={`text-sm font-semibold leading-6 ${
                  plan.popular ? 'text-gray-300 dark:text-gray-600' : 'text-gray-600 dark:text-gray-400'
                }`}>
              ...
              {getSavings(plan) && (
                <p className={`mt-2 text-sm ${
                  plan.popular ? 'text-green-300 dark:text-green-600' : 'text-green-600 dark:text-green-400'
                }`}>
