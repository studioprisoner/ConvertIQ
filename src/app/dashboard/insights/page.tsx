...
export default function AIInsightsPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (results.length > 0 && results.every((r: SearchResult) => r.similarity <= 0.5)) {
      ...
      if (error instanceof Error && error.message.includes('429')) {
        setIsUsingFallback(true);
        alert('Using keyword search due to API limits. For semantic search, please try again in a few minutes.');
      } else if (error instanceof Error && error.message.includes('Too Many Requests')) {
        alert('Search service is temporarily busy. Please try again in a few moments.');
      } else {
  ...
  const findSimilarReports = async (reportId: string) => {
    ...
    try {
      ...
      if (error instanceof Error && error.message.includes('429')) {
        ...
        alert('Service is temporarily busy. Please try again in a few moments.');
      } else {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <div className="mb-8">
        ...
        <Text className="mt-2">
          Discover patterns, find similar reports, and get intelligent insights from your analysis data.
        </Text>
      </div>
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-lg border border-zinc-950/10 dark:border-white/10 p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <Strong className="text-2xl">{insights.avgScore.toFixed(1)}</Strong>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-lg border border-zinc-950/10 dark:border-white/10 p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <Strong className="text-lg">{insights.topPatterns.length}</Strong>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-lg border border-zinc-950/10 dark:border-white/10 p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <Strong className="text-lg">{insights.commonIssues.length}</Strong>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-lg border border-zinc-950/10 dark:border-white/10 p-6 mb-8">
        ...
        <Text className="mb-4 text-zinc-600 dark:text-zinc-400">
          Search through your reports using natural language. Find insights based on meaning, not just keywords.
        </Text>
        
        <div className="flex gap-3">
          <InputGroup className="flex-1">
            <Input
              placeholder="e.g., 'mobile optimization issues' or 'conversion rate problems'"
              value={searchQuery}
        ...
        {isUsingFallback && (
          <div className="mt-4 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-700 rounded-lg">
            <Text className="text-amber-800 dark:text-amber-200 text-sm">
              ⚡ Using keyword search due to API rate limits. Results may be less precise than semantic search.
            </Text>
          </div>
        )}
              ...
              {searchResults.map((result) => (
                ...
                >
                  ...
                  {result.relevantSnippets?.slice(0, 2).map((snippet, idx) => (
                    <div key={idx} className="text-xs bg-zinc-50 dark:bg-zinc-800 rounded p-2 mb-2">
                      <Text className="text-zinc-700 dark:text-zinc-300">{snippet}</Text>
                    </div>
                  ))}
                </div>
              ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              {similarReports.map((report) => (
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <Badge color={getSimilarityColor(report.similarity)} size="sm">
                      {(report.similarity * 100).toFixed(1)}% similar
                    </Badge>
                  </div>
                  {report.commonTopics.length > 0 && (
                    <div className="mb-2">
                      <Text className="text-xs text-zinc-600 dark:text-zinc-400 mb-1">Common Topics:</Text>
        ...
        {insights && (
          <div className="bg-white dark:bg-zinc-900 rounded-lg border border-zinc-950/10 dark:border-white/10 p-6">
            ...
            {insights.topPatterns.length === 0 && insights.commonIssues.length === 0 && (
